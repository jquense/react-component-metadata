...
module.exports = function (state, opts) {
  ...
  return {
    enter: function (node, parent, scope) {
      ...
      if (isAssigning(node, "propTypes") && seenClass(component, scope)) parsePropTypes(resolveToValue(node.right, scope), json[component].props);else if (isAssigning(node, "defaultProps") && seenClass(component, scope)) parseDefaultProps(resolveToValue(node.right, scope), json[component].props, state.file);
    }
  };
};